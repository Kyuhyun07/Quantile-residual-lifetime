...
d.k=2
d.r.initial.0=(log(10/5)^(1/d.k))/d.exp.beta.initial.0
d.r.initial.1=(log(10/5)^(1/d.k))/d.exp.beta.initial.1
...
ismbwi.est = function(Z, nc, covariate, D, t_0, Q, ne){
  ...
  #fit = WKM(data[,2], (data[,(nc+4)]), zc = rep(1,n), w = rep(1,n))
  #fit = WKM(data[,1], (data[,(nc+4)]), zc = rep(1,n), w = rep(1,n))
  data2[,(nc+6)] = fit$jump*n2
  #data[,(nc+6)] = fit$jump*n
  #data[,(nc+6)] = fit$jump*n_t
  m = nrow(data2)
  ...
  objectF = function(beta){
    beta = as.matrix(beta)
    result = t(X*I) %*% {W*(pnorm((X%*%beta-logT)/sqrt(diag(X %*% H %*% t(X)))))-Q}
    #result = t(X*I) %*% {W*(pnorm((X%*%beta-logT)/sqrt(diag(X %*% H %*% t(X)))))-(W+1)*(Q/2)}
  }
  ...
  rev.objectF = function(beta){
    beta = as.matrix(beta)
    result = t(eta*X*I) %*% {W*(pnorm((X%*%beta-logT)/sqrt(diag(X %*% H %*% t(X)))))-Q}
    #result = t(eta*X*I) %*% {W*(pnorm((X%*%beta-logT)/sqrt(diag(X %*% H %*% t(X)))))-(W+1)*(Q/2)}
  }
  ...
  if (all(is.fit$fvec<1e-5&is.fit$fvec>-1e-5)){
    ...
    for (j in 1:ne){
      eta = rexp(n2,1)
      result = t(eta*X*I) %*% {W*(pnorm((X%*%solbeta-logT)/sqrt(diag(X %*% H %*% t(X)))))-Q}
      #result = t(eta*X*I) %*% {W*(pnorm((X%*%solbeta-logT)/sqrt(diag(X %*% H %*% t(X)))))-(W+1)*(Q/2)}
      result.ismb = cbind(result.ismb,result)
    }
    v = cov(t(result.ismb))
    a.beta = t(X*I*W*as.vector(dnorm((logT-X%*%solbeta)/
                                       sqrt(diag(X %*% H %*% t(X))))))%*%(-X/sqrt(diag(X %*% H %*% t(X))))
    inva.beta = qr.solve(a.beta)
    sigma = t(inva.beta) %*% v %*% inva.beta
    sd = sqrt(diag(sigma))
    beta.sd = cbind(solbeta, sd)
...
#   
#   # Covariate setting (1 covariate)
#   X = as.matrix(cbind(c(rep(1,m)),data2[,4:(nc+3)]))
#   W = data2[,(nc+6)]
#   logT = data2[,2]
#   I = data2[,3]
#   H = diag(1/n, nc+1, nc+1)
...
for (i in 1:2000){
  ...
  # Beta estimation method 3: ISMB & WO
  tryCatch({
    ismb.fit = ismbwi.est(a[,3], 1, a[,4], a[,5], 0, 0.5, 100)
    ...
    cover.is.00[i,8] = ind(0.6931472, cover.is.00[i,5], cover.is.00[i,7])}
    , error=function(e){
      b0.is.00[i] = NA
      b0.is.sd.00[i] = NA
      b1.is.00[i] = NA
...
for (i in 1:2000){
  ...
  tryCatch({
    ...
    cover.is.01[i,8]=ind(0.6931472,cover.is.01[i,5],cover.is.01[i,7])}
    , error=function(e){
      b0.is.01[i] = NA
      b0.is.sd.01[i] = NA
      b1.is.01[i] = NA
...
for (i in 1:2000){
  ...
  tryCatch({
    ...
    cover.is.03[i,8]=ind(0.6931472,cover.is.03[i,5],cover.is.03[i,7])}
    , error=function(e){
      b0.is.03[i] = NA
      b0.is.sd.03[i] = NA
      b1.is.03[i] = NA
...
for (i in 1:2000){
  ...
  tryCatch({
    ...
    cover.is.05[i,8]=ind(0.6931472,cover.is.05[i,5],cover.is.05[i,7])}
    , error=function(e){
      b0.is.05[i] = NA
      b0.is.sd.05[i] = NA
      b1.is.05[i] = NA
...
for (i in 1:2000){
  ...
  tryCatch({
    ...
    cover.is.07[i,8]=ind(0.6931472,cover.is.07[i,5],cover.is.07[i,7])}
    , error=function(e){
      b0.is.07[i] = NA
      b0.is.sd.07[i] = NA
      b1.is.07[i] = NA
...
for (i in 1:2000){
  ...
  # Beta estimation method 3: ISMB
  tryCatch({
    ismb.fit = ismbwi.est(a[,3], 1, a[,4], a[,5], 1, 0.5, 100)
    ...
    cover.is.10[i,8] = ind(0.7974189, cover.is.10[i,5], cover.is.10[i,7])}
    , error=function(e){
      b0.is.10[i] = NA
      b0.is.sd.10[i] = NA
      b1.is.10[i] = NA
...
for (i in 1:2000){
  ...
  tryCatch({
    ...
    cover.is.11[i,8]=ind(0.7974189,cover.is.11[i,5],cover.is.11[i,7])}
    , error=function(e){
      b0.is.11[i] = NA
      b0.is.sd.11[i] = NA
      b1.is.11[i] = NA
...
for (i in 1:2000){
  ...
  tryCatch({
    ...
    cover.is.13[i,8]=ind(0.7974189,cover.is.13[i,5],cover.is.13[i,7])}
    , error=function(e){
      b0.is.13[i] = NA
      b0.is.sd.13[i] = NA
      b1.is.13[i] = NA
...
for (i in 1:2000){
  ...
  tryCatch({
    ...
    cover.is.15[i,8]=ind(0.7974189,cover.is.15[i,5],cover.is.15[i,7])}
    , error=function(e){
      b0.is.15[i] = NA
      b0.is.sd.15[i] = NA
      b1.is.15[i] = NA
...
for (i in 1:2000){
  ...
  tryCatch({
    ...
    cover.is.17[i,8]=ind(0.7974189,cover.is.17[i,5],cover.is.17[i,7])}
    , error=function(e){
      b0.is.17[i] = NA
      b0.is.sd.17[i] = NA
      b1.is.17[i] = NA
...
for (i in 1:2000){
  ...
  # Beta estimation method 3: ISMB
  tryCatch({
    ismb.fit = ismbwi.est(a[,3], 1, a[,4], a[,5], 2, 0.5, 100)
    ...
    cover.is.20[i,8] = ind(0.9070615, cover.is.20[i,5], cover.is.20[i,7])}
    , error=function(e){
      b0.is.20[i] = NA
      b0.is.sd.20[i] = NA
      b1.is.20[i] = NA
...
for (i in 1:2000){
  ...
  tryCatch({
    ...
    cover.is.21[i,8]=ind(0.9070615,cover.is.21[i,5],cover.is.21[i,7])}
    , error=function(e){
      b0.is.21[i] = NA
      b0.is.sd.21[i] = NA
      b1.is.21[i] = NA
...
for (i in 1:2000){
  ...
  tryCatch({
    ...
    cover.is.23[i,8]=ind(0.9070615,cover.is.23[i,5],cover.is.23[i,7])}
    , error=function(e){
      b0.is.23[i] = NA
      b0.is.sd.23[i] = NA
      b1.is.23[i] = NA
...
for (i in 1:2000){
  ...
  tryCatch({
    ...
    cover.is.25[i,8]=ind(0.9070615,cover.is.25[i,5],cover.is.25[i,7])}
    , error=function(e){
      b0.is.25[i] = NA
      b0.is.sd.25[i] = NA
      b1.is.25[i] = NA
...
for (i in 1:2000){
  ...
  tryCatch({
    ...
    cover.is.27[i,8]=ind(0.9070615,cover.is.27[i,5],cover.is.27[i,7])}
    , error=function(e){
      b0.is.27[i] = NA
      b0.is.sd.27[i] = NA
      b1.is.27[i] = NA
...
for (i in 1:2000){
  ...
  # Beta estimation method 3: ISMB
  tryCatch({
    ismb.fit = ismbwi.est(a[,3], 1, a[,4], a[,5], 3, 0.5, 100)
    ...
    cover.is.30[i,8] = ind(1.0174711, cover.is.30[i,5], cover.is.30[i,7])}
    , error=function(e){
      b0.is.30[i] = NA
      b0.is.sd.30[i] = NA
      b1.is.30[i] = NA
...
for (i in 1:2000){
  ...
  tryCatch({
    ...
    cover.is.31[i,8]=ind(1.0174711,cover.is.31[i,5],cover.is.31[i,7])}
    , error=function(e){
      b0.is.31[i] = NA
      b0.is.sd.31[i] = NA
      b1.is.31[i] = NA
...
for (i in 1:2000){
  ...
  tryCatch({
    ...
    cover.is.33[i,8]=ind(1.0174711,cover.is.33[i,5],cover.is.33[i,7])}
    , error=function(e){
      b0.is.33[i] = NA
      b0.is.sd.33[i] = NA
      b1.is.33[i] = NA
...
for (i in 1:2000){
  ...
  tryCatch({
    ...
    cover.is.35[i,8]=ind(1.0174711,cover.is.35[i,5],cover.is.35[i,7])}
    , error=function(e){
      b0.is.35[i] = NA
      b0.is.sd.35[i] = NA
      b1.is.35[i] = NA
...
for (i in 1:2000){
  ...
  tryCatch({
    ...
    cover.is.37[i,8]=ind(1.0174711,cover.is.37[i,5],cover.is.37[i,7])}
    , error=function(e){
      b0.is.37[i] = NA
      b0.is.sd.37[i] = NA
      b1.is.37[i] = NA
